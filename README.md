...
are generated by running `gradle wrapper` locally, not hand-written.
The workflow sidesteps this by installing Gradle itself
(`gradle/actions/setup-gradle`) and calling `gradle assembleDebug`
directly instead of `./gradlew`. If you later work on this project
from a PC, run `gradle wrapper` once to add the wrapper files so
local builds don't need Gradle pre-installed either.
